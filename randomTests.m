...
t_ref = 0:Ts_ref:1;        % Time vector for reference signal
x_ref = cos(2*pi*f0*t_ref);% Reference signal

% Test sampling periods (convert ms to seconds)
...
n_ref = 0:50; % Plot first 50 samples for clarity
...
for i = 1:length(Ts_test)
    ...
    % Check if indistinguishable (matches 0.2 or 0.8)
    ...
    n = 0:min(50, length(x)-1); % Plot first 50 samples
    stem(n, x(1:length(n))), grid on;
    title(sprintf('T_s = %.3f ms: %s (f_{norm} = %.2f)', Ts*1000, result, f_norm_mod));
    xlabel('n'), ylabel('x[n]');
end