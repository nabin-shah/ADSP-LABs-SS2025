...
subplot(3,1,1);
plot(t*1e3, v0);
title('v_0(t) = 1.5 cos(2\pi 1kHz t)');
xlabel('Time (ms)'); ylabel('Amplitude (V)');

subplot(3,1,2);
plot(t*1e3, vc);
title('v_c(t) = 3 cos(2\pi 10kHz t)');
xlabel('Time (ms)'); ylabel('Amplitude (V)');

subplot(3,1,3);
plot(t*1e3, vout);
title('v_{out}(t) = max(v_0(t) + v_c(t), 0) (Diode Output)');
xlabel('Time (ms)'); ylabel('Amplitude (V)');

sgtitle('Mixer Output with Diode (Half-Wave Rectification)');