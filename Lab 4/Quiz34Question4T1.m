% --- Parameters ---
f0 = 50;          % Frequency of the continuous-time signal (Hz)
phi = pi/3;       % Phase of the signal
N = 1024;         % Number of samples (FFT points - power of 2 is efficient)
n = 0:N-1;        % Discrete time indices

% --- Case 1: fs = 500 Hz ---
fs1 = 500;        % Sampling frequency (Hz)
...
w0_1 = 2*pi * (f0/fs1);
disp(['Case 1 (fs = 500 Hz): Theoretical omega_0 = ', num2str(w0_1/pi), '*pi']);
disp(['Expected peaks (normalized) at +/- ', num2str(w0_1/pi)]);
...
title('DT Spectrum for fs = 500 Hz');
xlim([-1, 1]); % Limit x-axis to [-pi, pi]
ylim([0, max(abs(Xk1_shifted)/N) * 1.1]); % Adjust y-axis
...
disp(' ');
disp(['Case 2 (fs = 50 Hz): Theoretical omega_0 = ', num2str(w0_2/pi), '*pi']);
...
xlabel('\Omega / \pi');
ylabel('Magnitude |X(e^{j\Omega})|');
title('DT Spectrum for fs = 50 Hz');
xlim([-1, 1]);
ylim([0, max(abs(Xk2_shifted)/N) * 1.1]);

% --- Case 3: fs = 40 Hz ---
fs3 = 40;         % Sampling frequency (Hz)
...
disp(' ');
disp(['Case 3 (fs = 40 Hz): Theoretical omega_0 = ', num2str(w0_3/pi), '*pi']);
...
xlabel('\Omega / \pi');
ylabel('Magnitude |X(e^{j\Omega})|');
title('DT Spectrum for fs = 40 Hz');
xlim([-1, 1]);
ylim([0, max(abs(Xk3_shifted)/N) * 1.1]);